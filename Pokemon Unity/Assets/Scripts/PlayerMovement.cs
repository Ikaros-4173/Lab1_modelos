...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()//aqui se setea los movimientos por teclado....modificar aqui con un programa random :v 
    {
        ...
        {
            System.Random aleatorio = new System.Random();//esta funcion moverá al bichareco de manera random en el mapa
            direccion = aleatorio.Next(0, 400);
            if (direccion >= 0 && direccion <=99)
            {
                transform.Translate(Vector3.up * speed * Time.deltaTime);
            }
            if (direccion >= 100 && direccion <=199)
            {
                transform.Translate(Vector3.down * speed * Time.deltaTime);
            }
            if (direccion >= 200 && direccion <=299)
            {
                transform.Translate(Vector3.right * speed * Time.deltaTime);
            }
            if (direccion >= 300 && direccion <=400)
            {
